...
// 基于单链表实现lru算法 leetcode  https://leetcode-cn.com/problems/lru-cache/description/
...
func (this *LRUCache) Print() {
	cur := this.Data
	format := fmt.Sprintf("Length:%d,Capacity:%d,Data:", this.Length, this.Capacity)
	for nil != cur {
		format += fmt.Sprintf("[key:%d,value:%d]", cur.key, cur.value)
		cur = cur.next
		if nil != cur {
			format += "->"
		}
