...
因为我们的算法要求一个最大值，所以初始值设置为0,方便取最大值
...
func maxProfit1(k int, prices []int) int {
	if k < 1 || len(prices) < 2 { //不允许交易或者不足以完成一次交易时,最大利润都为0
		return 0
	}
...
2. 第3维只有0和1两种状态也可以考虑把第3维和第2维互换,用用2个长度为k+1的1维数组表示 dp
dp[0]表示不持有股票时的最大利润, dp[1]表示持有股票时的最大利润
...
func maxProfit(k int, prices []int) int {
	...
	// dp[0]表示不持有股票时的最大利润, dp[1]表示持有股票时的最大利润
	dp := [2][]int{make([]int, k), make([]int, k)}
